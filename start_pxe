...
sudo sed -i '/^dhcp-range=192.168.0.0,proxy/d' /etc/dnsmasq.conf
...
sudo dnsmasq --enable-tftp --tftp-root=$PXEDATA/boot --dhcp-boot=pxelinux.0,"$IP",$IP --dhcp-range=$(echo $IP | cut -d. -f1-3).50,$(echo $IP | cut -d. -f1-3).250,infinite --log-dhcp
